...
lmaxapwlo = 3
Nlmo = lmaxapw + lmaxapwlo

ALLOCATE( uju( 0:lmaxexp,0:Nlmo,0:Nlmo,nuju ))
uju=0.d0
  ...
  do l3=0,lmaxexp
     ...
     do l1= 0, lmaxapw
        ...
        do l2 = l1, lmaxapw ! Upper triangle
           ...
           ! uju
           do ir=1,nrmt(is)
              fr(ir) = ufr(ir,l1,1,ic) * ufr(ir,l2,1,ic) * jl(ir,l3)
           enddo !ir
           uju(l3,l1,l2,i1) = rintegrate( nrmt(is), spr(1,is), fr )
           uju(l3,l2,l1,i1) = uju(l3,l1,l2,i1)
           ! ujuflg(l3,l1,l2) = .true.
           ! ujuflg(l3,l2,l1) = .true.
...
call mpi_grid_reduce( uju(0,0,0,1), (lmaxexp+1)*(Nlmo+1)**2*nuju, &
                      dims=(/dim_k/), all=.true. )
...
!     for atom classes only (not for all atoms)
     ...
     do l1 = 0, Nlmo
        do m1 = -MOD(l1,lmaxapw), MOD(l1,lmaxapw) 
           lm1 = idxlm( MOD(l1,lmaxapw), m1 )

           do l2 = 0, Nlmo
              do m2 = -MOD(l2,lmaxapw), MOD(l2,lmaxapw)
                 lm2 = idxlm( MOD(l2,lmaxapw), m2 )

                 zt1 = zzero
                 do l3 = 0, lmaxexp
                    zt1 = zt1 + zm(l3,lm2,lm1) * uju( l3,l1,l2,(ic-1)*ngqsh(iq)+gqshidx(ig,iq) ) 
                 enddo ! l3

                 gntuju( lm2, lm1, ic,ig) = zt1
