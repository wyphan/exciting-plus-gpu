...
complex(8), intent(out) :: wfsvmt( ngntujumax ,natmtot,nspinor,nwf)
...
allocate( wfmt( ngntujumax, natmtot,nwf))
n = ngntujumax*natmtot
do ispn=1,nspinor
  j=(ispn-1)*nmatmax
  ...
  do ias=1,natmtot
    ordl=0
    is=ias2is(ias)
    do l = 0, lmax + lmaxapwlo
       do m = -MOD(l,lmax), MOD(l,lmax)
          lm = idxlm( MOD(l,lmax), m )
          wfsvmt(lm,ias,ispn,:)=wfmt(lm,ias,:)
       enddo !m
       ordl(l)=apword(l,is)
    enddo !l
    do ilo = 1, nlorb(is)
       WRITE(*,*) 'Need to address', ilo
